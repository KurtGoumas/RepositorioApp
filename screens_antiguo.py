...
class Procesado(tk.Frame):
    ...
    def init_widgets(self): #Aqui van los botones y demas 
        ...
        posicion= {'Select': [0,0],'CSV':[1,0], 'Titulo param': [0,0], 'Camara 1': [1,0], 'Camara 2': [2,0], 'N Objetos': [3,0], 'Peso Mov': [4,0], 'Lx': [3,2], 'Ly': [4,2], 'w start':[3,4], 'h start':[4,4] }
        ...
        texto_ws= tk.Label(etiquetasFrame,
                            text='w_s: ',
                            **style.STYLE
                              
        ).grid(row=posicion['w start'][0],column=posicion['w start'][1])

        texto_wl= tk.Label(etiquetasFrame,
                            text='w_l: ',
                            **style.STYLE
                              
        ).grid(row=posicion['w start'][0],column=posicion['w start'][1]+2)

        texto_hs= tk.Label(etiquetasFrame,
                            text='h_s: ',
                            **style.STYLE
                              
        ).grid(row=posicion['h start'][0],column=posicion['h start'][1])

        texto_wl= tk.Label(etiquetasFrame,
                            text='h_l: ',
                            **style.STYLE
                              
        ).grid(row=posicion['h start'][0],column=posicion['h start'][1]+2)
        ...
        w_start= tk.Entry(etiquetasFrame,
                          **style.STYLE,
                          textvariable= self.w_start,
                          width = self.text_entry_width

        ).grid(row= posicion['w start'][0], column= posicion['w start'][1]+1)

        w_last= tk.Entry(etiquetasFrame,
                          **style.STYLE,
                          textvariable= self.w_last,
                          width = self.text_entry_width

        ).grid(row= posicion['w start'][0], column= posicion['w start'][1]+3)

        h_start= tk.Entry(etiquetasFrame,
                          **style.STYLE,
                          textvariable= self.h_start,
                          width = self.text_entry_width

        ).grid(row= posicion['h start'][0], column= posicion['h start'][1]+1)

        h_last= tk.Entry(etiquetasFrame,
                          **style.STYLE,
                          textvariable= self.h_last,
                          width = self.text_entry_width

        ).grid(row= posicion['h start'][0], column= posicion['h start'][1]+3)
